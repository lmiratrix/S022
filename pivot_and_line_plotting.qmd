# Using pivot methods to make line graphs {#pivot_and_line_plotting}
...
The key idea is if we have multiple columns, and we want a line for each column, we need to pivot our data long, so we can make each column a distinct group, and plot all the lines using a single `geom_line()` command.
