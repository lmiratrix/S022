...
Uses the `here()` function to set relative file paths to locations in the project directory. 

```{r eval=FALSE}
school_dat <- read_csv(glue("{here::here()}/data/school_data_2025.csv"))
...
# Cleaning code here
saveRDS(clean_dat, glue("{here::here()}/data/clean_school_data.Rds"))
```
