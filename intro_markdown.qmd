...
When working with R Markdown, we highly recommend that you keep the [Posit Cheatsheet on R Markdown](https://rstudio.github.io/cheatsheets/rmarkdown.pdf) close at hand—this is a great resource showing the various options and syntax you can use to customize your knitted document.

## Getting started: Formatting Your Document
...
### Formatting Individual Code Chunks
...
````md
```{r eval = FALSE}`r ''`
# The document will knit even if the code in this chunk has an error 

# For example, the "bad" code below doesn't prevent the document from knitting
my_var <- !!198`123bx92
```
````
...
Ultimately, the output of each code chunk is highly customizable. For granular information about how to format your chunk output exactly as desired, please take a look at [Ch. 11 of the R Markdown Cookbook by Xie, Dervieux, & Riederer (2024)](https://bookdown.org/yihui/rmarkdown-cookbook/chunk-options.html).

### Setting Global Options Code Chunks

Instead of specifying code chunks options every time, you can specify them globally in the setup chunk by using the `knitr` package. The code below sets the new default for code chunks in your project. Note that, because this chunk uses `echo=FALSE` in its header, this setup code will be hidden in the final rendered document. 
...
```
````
You can then add additional options **only** to relevant chunks using the chunk headers. For example, if you want to exclude specific chunks, you can re-set `echo = FALSE` and `eval = FALSE` for those specific chunks.
...
To add inline code, enclose it in `` `r ` ``. For example, to report the mean reading score, you can use `` `r knitr::inline_expr('round(mean(dat$p7read))')` ``, which will produce the number `r round(mean(dat$p7read),2)` in your rendered document. Here we used two commands: `round` and `mean`. You can use more commands and write more complex inline code, depending on what you want to report.

## Embedding plots

Plots are easy to embed. For example, I use the chunk headings in the example below to control the width of the displayed output (75% of the full document width) using `out.width='75%'`. I also turn off pesky warnings and messages (since I know I can safely ignore them) to avoid cluttering the output using `message=FALSE` and `warnings=FALSE`. 

````md
```{r out.width='75%', message=FALSE, warning=FALSE`r ''`
library(ggplot2)
...
```{r, echo = FALSE, out.width='75%', message=FALSE, warnings=FALSE}
library(ggplot2)
...
Girls are rendered as coral, boys are rendered in turquoise, and the line of best fit is drawn in `darkorchid3` (because why not). Just because you have a lot of colors and plotting characters to work with doesn't mean you need to use them all. Notice that this command draws on `dat`, which we loaded in a previous chunk. When knitting the document, all code chunks with the default `eval=TRUE` option get executed in order and the results persist throughout the R Markdown document.
